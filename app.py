...
def precompute_floyd_warshall(graph):
    start_time = time.time()
    distance_matrix = dict(nx.floyd_warshall(graph, weight="weight"))
    precomputed_time = time.time() - start_time
    precomputed_memory = psutil.Process().memory_info().rss
    return distance_matrix, precomputed_time, precomputed_memory

floyd_warshall_matrix, floyd_time, floyd_memory = precompute_floyd_warshall(G)

# Find the shortest path using Floyd-Warshall
def find_shortest_path_floydwarshall(source, destination):
    source_coords = ports[source]
    destination_coords = ports[destination]
    ...
    shortest_path = nx.reconstruct_path(source_nearest, destination_nearest, floyd_warshall_matrix)
    return shortest_path, floyd_time, floyd_memory

# (Existing functions for Dijkstra, A*, Bellman-Ford remain unchanged)

# Function to generate comparative graphs using Plotly
def generate_comparative_graph(dijkstra_time, astar_time, bellmanford_time, floyd_time,dijkstra_memory, astar_memory, bellmanford_memory, floyd_memory):
    # Time comparison graph
    time_data = [dijkstra_time, astar_time, bellmanford_time, floyd_time]
    time_labels = ['Dijkstra', 'A*', 'Bellman-Ford', 'Floyd-Warshall']

    time_fig = go.Figure([go.Bar(x=time_labels, y=time_data, marker=dict(color=['green', 'blue', 'red', 'purple']))])
    ...
    memory_data = [dijkstra_memory, astar_memory, bellmanford_memory, floyd_memory]
    memory_fig = go.Figure([go.Bar(x=time_labels, y=memory_data, marker=dict(color=['green', 'blue', 'red', 'purple']))])
    memory_fig.update_layout(title='Space Complexity Comparison', xaxis_title='Algorithm', yaxis_title='Memory Usage (bytes)')

    # Convert to HTML for embedding in templates
...
def index():
    ...
    if request.method == "POST":
        ...
        if source in ports and destination in ports:
            ...
            floyd_path, floyd_time, floyd_memory = find_shortest_path_floydwarshall(source, destination)
            ...
            performance_metrics = {
                ...
                "floyd_memory": floyd_memory,
            }

            # Plot the route using Dijkstra (default choice for visualization)
            map_html = plot_route(source, destination, dijkstra_path)

            # Determine the best algorithm based on time
            times = {"Dijkstra": dijkstra_time, "A*": astar_time, "Bellman-Ford": bellmanford_time, "Floyd-Warshall": floyd_time}
            best_algorithm = min(times, key=times.get)

            time_taken = ", ".join([f"{algo}: {time:.6f}s" for algo, time in times.items()])
            memory_used = ", ".join([f"{algo}: {memory} bytes" for algo, memory in zip(times.keys(), [dijkstra_memory, astar_memory, bellmanford_memory, floyd_memory])])
...
def analysis():
    ...
    floyd_time = performance_metrics["floyd_time"]
    ...
    floyd_memory = performance_metrics["floyd_memory"]

    # Generate comparison graphs
    time_graph, memory_graph = generate_comparative_graph(
        dijkstra_time, astar_time, bellmanford_time, floyd_time,
        dijkstra_memory, astar_memory, bellmanford_memory, floyd_memory
    )
